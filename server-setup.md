...
## Precinct Wireguard
We will call the interface for Precinct tunneling `wg1`. Keys are generated in the server and then we put them in the server's configuration file via `sudo nano /etc/wireguard/wg1.conf`. IPs and Port shall not overlap with MOSIP Wireguard.

We also created a configuration file (get from dev team) for a PrecinctOfficer connecting to the server. 
